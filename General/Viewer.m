...
%         end
        ...
        function status=loadOrStart(A,message)
            % Choose whether to start a new object or load an existing one.
            % Returns string 'start','load','cancel';
            % 
            % Note that start and cancel both leave the object unchanged -
            % it's up to the caller to implement the difference.
            
            if ~exist('message','var'), 
                message=sprintf('Start a new %s object or load an existing one?',class(A));
            end
            
            res=questdlg(message,class(A),'Start New','Load Existing','Cancel','Start New');            
            switch res
                case 'Load Existing'
                    gotit=A.Load;
                    if isempty(gotit),
