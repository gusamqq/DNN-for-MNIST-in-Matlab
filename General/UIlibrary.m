...
function [h valfun]=buttons(varargin)
% Syntax
...
% ~label@3                  - index selector (not made yet)
...
for i=1:length(list)
  ...
  elseif list{i}(1)=='~' && any(list{i}=='@') % Integer selector
      ...
      maxx=str2num(list{i}(div+1:end));
      
      
      
      assert(~isempty(maxx),'Must include a maximum number in range');
      
      
      set(gcf,'unit','character');
      
      loc=locFun(i);
      
%       bw=loc(3)*.2; % button width
%       hc=[hc uicontrol('style','text','Units','characters','string',sprintf('%s (%g)',list{i}(2:div-1),maxx),'position',loc+[0 1 0 0])];   
      hc=[hc uicontrol('style','text','Units','characters','position',loc+[0 1 0 0])];      
      
      if maxx<=1
          h(i)=uicontrol('Style', 'slider','Position', loc,'enable','off');
      else
          h(i)=uicontrol('Style', 'slider','value',1,'Units','characters','Position', loc,'min',1,'max',max(maxx,1+eps),'SliderStep',[max(1/(maxx-1),eps) .1]);
      end
      ...
      setsample=@(k)set(hc(end),'string',sprintf('%s %g/%g',list{i}(2:div-1),k,maxx));
      
      hL=addlistener(h(i),'Value','PostSet',@(e,s)setsample(get(s,'NewValue')));
      valuefun{i}=@()round(get(h(i),'value'));
      
      setsample(1);
      
%       h(i)=uicontrol('Style', 'edit', 'string',list{i}(div+1:end),'Units','characters','Position', loc+[bw 0 -2*bw 0]);
      
%       butloc=loc; butloc(3)=bw;
%       hc=[hc uicontrol('string','<','callback',@(e,s)set(h(i),'string',num2str(min(str2num(get(h(i),'string',-1)),1))),'position',butloc)];      
%       hc=[hc uicontrol('string','>','callback',@(e,s)set(h(i),'string',num2str(max(str2num(get(h(i),'string',+1)),Inf),'position',butloc+[loc(3)-2*bw 0 0 0])))];
...
% Transform valuefun to more practical form
valfun=@(ix)valuefun{ix}();
...
% points with the mouse, in the current axis;
    ...
    if ~exist('hL','var'), hL=[nan nan]; end
...
function [h hF]=figtype(type,N,varargin)
...
%             set(gcf,'Position',[100 200 screen(1)-100 min(screen(1)/N,screen(2)-300)]);
        ...
        otherwise
            error ('Figure type "%s" is not supported',type);
%             h=[];
    end
...
function [s hB]=checkMenu(options,initial,varargin)
    ...
    %
    % Example use:
    % s=checkMenu({'foo','bar'},[true false],hAxis,'callback',@(s,e)dosomthing);
    % s.foo()
    % s.bar()
    %
    % woohoo

    % Handle additional callback
    ix=find(strcmpi(varargin,'callback'));
    if any(ix)
    ...
    % Enable boolean-vector output option
    ix=find(strcmpi(varargin,'-bool'),1);
    if any(strcmpi(varargin,'-bool'))
        boolout=true;
        varargin(ix)=[];
    else
        boolout=false;
    ...
    if ~isempty(varargin) && ishghandle(varargin{1}(1))
        hB=varargin{1};
        varargin=varargin(2:end);
        set(hB,'style','popupmenu',varargin{:},'callback',@(s,e)selectItem(s,e),'value',1);        
    else
        hB=uicontrol('style','popupmenu','callback',@(s,e)selectItem(s,e),'value',1,varargin{:});
    end
    ...
    if boolout
        ...
        function L=refreshlist
            L=cellfun(@(tg,op)sprintf('%c %s',char(43+2*tg),op),num2cell(tog),options,'uniformoutput',false);
            set(hB,'string',L);
        end
...
% 
%         function L=refreshlist
%             L=cellfun(@(tg,op)sprintf('%c %s',char(32+88*tg),op),num2cell(tog),options,'uniformoutput',false);
%             set(hB,'string',L);
%         end
% 
%         function selectItem
...
function val=controls(controlStruct,blocking)
% controlStruct is a structure array with fields:
%
% Field     Default     Description     
% name      -           Name of control
% type      -           Any of 'int','real','string','list','checklist','bool';
% scalar    true        Whether a scalar value is required
% default   -           Default value to put in there
% enum      -           In case of list, the options
% 
% Example Use
% cs(1).name='stringthing';
% cs(1).type='string';
% sc(2).name='num';
% cs(2).name='num';
% cs(2).type='real';
...
% cs(4).enum={'aa','bb','ccc'};
% 
% val=UIlibrary.controls(cs);
    
    
    if ~exist('blocking','var'), blocking=true; end

    cs=controlStruct;

    if ~isfield(cs,'default'), cs(1).default=[]; end
    
    hF=figure('name','controls','MenuBar','none','Position',[500 500 400 200]);

    % Build controls
    spacing=0.8/length(cs);
    height=0.5/length(cs);
    namewid=.3;
    gap=.1;
    ...
    for i=1:length(cs)
        ...
        % Build Controls list
        uicontrol('style','text','string',[cs(i).name ' '],'units','normalized','position',[loc namewid height],'FontWeight','bold','HorizontalAlignment','right');
        hc(i)=uicontrol('string',cs(i).name,'units','normalized','position',[loc+[namewid+gap 0] 1-namewid-2*gap height]);
        switch cs(i).type
            case 'bool'
                set(hc(i),'style','checkbox');
                if ~isempty(cs(i).default), set(hc(i),'value',cs(i).default); end
            case {'string','int','real'}
                set(hc(i),'style','edit');
                if ~isempty(cs(i).default), set(hc(i),'string',num2str(cs(i).default)); end
            case 'list'
                if cs(i).scalar
                    set(hc(i),'style','popupmenu','string',cs(i).enum);
                    if ~isempty(cs(i).default), set(hc(i),'value',cs(i).default); end
                else
                    [spec{i} hB]=UIlibrary.checkMenu(cs(i).enum,cs(i).default,'-bool',hc(i),'units','normalized','position',get(hc(i),'position'));
%                     delete(hc(i));
%                     hc(i)=hB;
%                     
                end
    ...
    uicontrol('units','normalized','position',[0 0 .5 height],'string','ok','callback',@(s,e)ok);
    uicontrol('units','normalized','position',[.5 0 .5 height],'string','cancel','callback',@(s,e)cancel);
    ...
    function v=readvals
        ...
        for j=1:length(cs)
            switch cs(j).type
                ...
                case 'list'
                    if cs(j).scalar
                        v{j}=cs(j).enum{get(hc(j),'value')};
                    else
                        v{j}=cs(j).enum(spec{j}()); % sloppeh!
                    end
                case 'string'
                    v{j}=get(hc(j),'string');
                case {'int','real'}
                    v{j}=str2num(get(hc(j),'string'));
            end
        end
