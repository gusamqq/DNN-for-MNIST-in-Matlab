...
if strcmp(token,'.')
    superlinks=cellfun(@(x)sprintf('<a href="matlab:edit %s">%s</a>',x,x),superclasses(class),'uniformoutput',false);
    if isempty(superlinks)
        fprintf('Class <a href="matlab: edit %s">%s</a>\n',class,class);
    else
        superlinks=strcat(superlinks,', ');
        superlinks=strcat(superlinks{:});
        fprintf('Class <a href="matlab: edit %s">%s</a> is a subclass of %s\n',class,class,superlinks(1:end-1));
    end
end
